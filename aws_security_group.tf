# ALB
resource "aws_security_group" "main-vpc-alb" {
  name        = "${lookup(var.settings, "app_name")}-sg-alb-${terraform.workspace}"
  description = "For ${terraform.workspace} of main-vpc-alb"
...
resource "aws_security_group_rule" "main-vpc-alb-1" {
  security_group_id = "${aws_security_group.main-vpc-alb.id}"
  type              = "ingress"
  from_port         = 80
  to_port           = 80
  protocol          = "tcp"
...
resource "aws_security_group_rule" "main-vpc-alb-2" {
  security_group_id = "${aws_security_group.main-vpc-alb.id}"
  type              = "ingress"
  from_port         = 443
  to_port           = 443
...
resource "aws_security_group_rule" "main-vpc-alb-3" {
  security_group_id = "${aws_security_group.main-vpc-alb.id}"
  type              = "egress"
  from_port         = 0
  to_port           = 0
...
resource "aws_security_group_rule" "main-vpc-web-1" {
  ...
  to_port                  = 80
  protocol                 = "tcp"
  source_security_group_id = "${aws_security_group.main-vpc-alb.id}"
}
